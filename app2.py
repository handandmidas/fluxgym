...
function () {
  ...
  const $ = (sel, root = getRoot()) => root.querySelector(sel);

  function onAvailable(selector, cb) {
    const root = getRoot();
  ...
  // Autoscroll
  onAvailable("#autoscroll", (autoscroll) => {
    if (window.iidxx) window.clearInterval(window.iidxx);
    window.iidxx = window.setInterval(function () {
      const root = getRoot();
      const line = root.querySelector(".codemirror-wrapper .cm-line");
  ...
  // Auto-refresh dentro del shadow root
  const debouncedClick = debounce(() => {
    const refreshBtn = $("#refresh");
    if (refreshBtn) refreshBtn.click();
  }, 1000);

  getRoot().addEventListener("input", debouncedClick, { capture: true });

  // Efecto visual Start cuando exista
  onAvailable("#start_training", (btn) => {
